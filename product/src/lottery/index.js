  ...
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    switch (target) {
      ...
      case "reLottery":
        ...
        if (Array.isArray(EACH_COUNT[currentPrizeIndex])) {
          // 如果不存在
          currentPrizeChildIndex -= 1
        }
...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    if (Array.isArray(EACH_COUNT[currentPrizeIndex])) {
      // 如果不存在
      perCount = EACH_COUNT[currentPrizeIndex][currentPrizeChildIndex];
      currentPrizeChildIndex += 1;
    } else {
...
function changePrize() {
  ...
  if (Array.isArray(EACH_COUNT[currentPrizeIndex])) {
    luckyCount = (luckys ? luckys.length : 0) + EACH_COUNT[currentPrizeIndex][currentPrizeChildIndex];
  } else {
    luckyCount = (luckys ? luckys.length : 0) + EACH_COUNT[currentPrizeIndex];
  }

  console.log('🚀 --> file: index.js:718 --> changePrize --> luckyCount:', luckyCount);
...
function changeCard(cardIndex, user) {
  ...
  card.innerHTML = `<div class="avator"><img src="${avatorUrl}" /></div><div class="name">${user[1]
    }</div><div class="details">${greeting || ""}</div>`;
}
